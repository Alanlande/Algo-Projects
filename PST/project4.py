...
"""
Prim's Algorithm: use Prim's MST algorithm to update the graph
INPUT: 
    adjList, denotes the vertex list
    adjMat, carries the edge values
OUTPUT: None
"""
def prim(adjList, adjMat):
    ...
    start = adjList[randint(0, len(adjList) - 1)]
    start.cost = 0
    # Make the priority queue using cost for sorting.
    minQueue = MinQueue()
    # insert the start into the minQueue
    minQueue.insert(start)
    while not minQueue.isEmpty():
        # Get the next unvisited vertex and visit it.
        v = minQueue.deleteMin()
        v.visited = True
        # For each edge out of v.
        for neighbor in v.neigh:
            # make sure this neighbor is unvisited
            if not neighbor.visited:
                # update only if unvisited and has the min distance
                if neighbor.cost > adjMat[v.rank][neighbor.rank]:
                    neighbor.cost = adjMat[v.rank][neighbor.rank]
                    # assigned the proper vertex.prev
                    neighbor.prev = v
                    # Update minQueue
                    minQueue.insert(neighbor)
    return

################################################################################

"""
Kruskal's Algorithm: use Kruskal's MST algorithm to find the MST list
INPUT: 
    adjList, the vertex list
    edgeList, the sorted edge list
...
OUTPUT:
    tour, the travelling trace
Note: tour contains all the rank instead of vertex objects
"""
def tsp(adjList, start):
    # Flag all vertices as unvisited.
    for vertex in adjList:
        vertex.visited = False
    # declare tour list and stack myStack to DFS
    tour = []
    myStack = []
    # push the first city into the stack
    myStack.append(start)
    while len(myStack) != 0:
        current = myStack.pop()
        # skip the visited city
        if current.visited:
            continue
        # mark current visited city's rank and add it to the tour list
        current.visited = True
        tour.append(current.rank)
        # push current city's neighbors into the stack
...
Disjoint Set Functions:
    makeset, create a singleton set containing vertex v
    find, find the root vertex
    union, union two vertexes
...
INPUT: v and u, two vertexes that are about to be union
OUTPUT: None
Note: Use the isEqual method of the Vertex class when comparing vertices.
"""
def union(u,v):
