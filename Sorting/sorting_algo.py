...
"""
SelectionSort: sort input list A using selection sort, return sorted list A
"""
def SelectionSort(A):
    n = len(A)
    # [0, i] maintains the sorted part, [i + 1, n - 1] maintains the unsorted
    for i in range(n):
        ...
        for j in range(i + 1, n, 1):
            if A[minIndex] > A[j]:
                minIndex = j
        # swap the found min with the end of sorted array
        A[i], A[minIndex] = A[minIndex], A[i]
    return A

"""
InsertionSort: sort input list A using insertion sort, return sorted list A
"""
def InsertionSort(A):
    n = len(A)
    # [0, i] maintains the sorted part, [i + 1, n - 1] maintains the unsorted
    for i in range(1, n, 1):
        # shift numbers in A[0..i-1] that A[j] > pivot right by 1
        # insert pivot at the correct position
        # shift numbers greater than pivot by 1 to the right
        pivot = A[i]
        j = i - 1
...
"""
BubbleSort: sort input list A using bubble sort, return sorted list A
"""
def BubbleSort(A):
    n = len(A)
    for i in range(n):
        ...
        for j in range(1, n - i, 1):
            ...
            if A[j - 1] > A[j]:
                A[j - 1], A[j] = A[j], A[j - 1]
                unsortedFlag = True
        # All numbers are sorted already, exit
        if unsortedFlag is False:
            break
    return A

"""
MergeSort: sort input list A using merge sort, return sorted list A
"""
def MergeSort(A):
    if len(A) < 2:
        return A
    MyMergeSort(A, 0, len(A) - 1, list(A))
    return A


"""
MyMergeSort: merge sort A inplace
start and end indicate the sublist boundary, temp is an allocated 
same-length list for copying sorted numbers
"""
def MyMergeSort(A, start, end, temp):
    if start >= end:
        return
    mid = (start + end) // 2
    # divide the sorting into two n/2 equal-length sorting problem
    MyMergeSort(A, start, mid, temp)
    MyMergeSort(A, mid + 1, end, temp)
    # conquer the two seperated sorted parts
    merge(A, start, end, temp)

"""
merge merge two sorted parts of A
"""
def merge(A, start, end, temp):
    ...
    rightStart = mid + 1
    index = start
    # put the current smallest number at the end of unsorted parts
    while leftStart <= mid and rightStart <= end:
        if A[leftStart] < A[rightStart]:
            ...
            rightStart += 1
            index += 1
    # corner case: traversed rightStart already
    while leftStart <= mid:
        temp[index] = A[leftStart]
        leftStart += 1
        index += 1
    # corner case: traversed leftStart already
    while rightStart <= end:
        temp[index] = A[rightStart]
        rightStart += 1
        index += 1
    # copy sorted subarray into original array A
    for i in range(start, end + 1, 1):
        A[i] = temp[i]


"""
QuickSort: Sort a list A with the call QuickSort(A, 0, len(A)).

"""
def QuickSort(A, i, j):
    if j <= 1:
        return A
    # j - 1 is the last index
    MyQuickSort(A, i, j - 1)
    return A
...
Sort a list A inplace with the call MyQuickSort(A, 0, len(A) - 1).
"""
def MyQuickSort(A, start, end):
    if start >= end:
        return
    ...
    # numbers indexed smaller than left are all < pivot
    # numbers indexed greater than right are all > pivot
    while left <= right:
        ...
        if left <= right:
            ...
            left += 1
            right -= 1
    # recursively sort the rest unsorted subarray
    MyQuickSort(A, start, right)
    MyQuickSort(A, left, end)
